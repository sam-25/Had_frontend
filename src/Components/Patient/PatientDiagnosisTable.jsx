  ...
  const navigate = useNavigate();  // Adding useNavigate hook
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:8080/patient/getConsultations`, {
          params: {
        ...
        <table {...getTableProps()} className="w-full border-collapse border">
          ...
          <tbody {...getTableBodyProps()}>
            {rows.map(row => {
              prepareRow(row);
              return (
                <tr {...row.getRowProps()} className="hover:bg-gray-100 cursor-pointer" onClick={() => navigate(`/PatientConsultation/${row.original.diagnosisId}`)}>
                  {row.cells.map(cell => {
                    return (
